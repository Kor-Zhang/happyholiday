...
var title = undefined;
//新闻信息显示区域
var newsContent = undefined;
//储存远程加载的所有的新闻信息
var allNews = undefined;
//储存返回的数据
...
$(function($){
	/*******************初始化dom*****************/
	newsContent = $('#officialwebsite_news_displayArea_right_content');
	titleUl = $('#officialwebsite_news_displayArea_left>ul');
	title = $('#officialwebsite_news_displayArea_right_title');
	createTime = $('#officialwebsite_news_displayArea_right_createTime');
	nextPageBtn = $('#officialwebsite_news_displayArea_left_nextPage>a');
	prePageBtn = $('#officialwebsite_news_displayArea_left_prePage>a');
	//远程加载新闻
	loadNews(page,rows);
...
function loadNews(page,rows){
	...
	createTime.html("");
	//加载数据
	zkAjax('/ow/owNewsAction!getNews.action?page='+page+"&rows="+rows,function(data){
		retData = data;
		allNews = data.rows;
		//加载显示标题列表
		for (var index = 0; index < allNews.length; index++) {
			var news = allNews[index];
			//展示的字符串
			var displayTitle = splitBeyondStr(titleLimitLength,news.title);
			//a便签保存题目的完整信息，保存这条新闻记录的id（idVal）
			var a = $("<a title="+news.title+" idVal='"+news.id+"' onClick=showNewsContent('"+news.id+"') href=javascript:void(0);>"+displayTitle+"</a>");
			var li = $('<li></li>');
			li.append(a);
			titleUl.append(li);
		}
		...
		if(totalPage<nextPage){
			...
			//设置按钮的触发事件
			nextPageBtn.attr('href',"javascript:loadNews("+nextPage+","+rows+")");
		}
		//如果是第一页
		if(prePage<1){
			...
			//设置按钮的触发事件
			prePageBtn.attr('href',"javascript:loadNews("+prePage+","+rows+")");
		}
		
		//默认显示的新闻
		showNewsContent(allNews[0].id);
	});
}
...
function showNewsContent(newsId){
	for (var index = 0; index < allNews.length; index++) {
		...
		if(newsId==news.id){
			...
			if(currtTitleANode!=undefined&&news.id==$(currtTitleANode).attr('idVal')){
				break;
			}
			...
			//替换标题列表的缩略标题为完整标题
			$(titleUl.find('a')[index]).html(news.title);
			//显示穿件时间
