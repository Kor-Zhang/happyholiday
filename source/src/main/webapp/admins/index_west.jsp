		...
		<div title="系统管理" data-options="">
			<ul id="menuTree" class="easyui-tree"
				data-options="lines:true,
						url:'${pageContext.request.contextPath}/menuTreeAction!getTree.action',
...
<script type="text/javascript">
	...
	*/
	function loadSystemddlTree(){
		...
		//连接服务器判断是否能查询字典
		zkAjax('/systemddlAction!getSystemddlByParentId.action',function(data){
			//显示信息
			showMsg(data);
			//检测是否在线
			try{
	...
	*/
		...
		systemddlETree = $('#admins_index_systemddl').tree({    
		    ...
		    onBeforeExpand:function(node){
		    	...
		    	if(nodeLength==0){
					...
					$.ajax({
				     type: "POST", 
						url:getWebProjectName()+'/systemddlAction!getSystemddlByParentId.action?id='+clickNode.id,
						dataType:"json",
						async:false,
			...
			onAfterEdit:function(node){
				...
				//发出携带信息的ajax
				zkAjaxData("/systemddlAction!updateSystemddl.action",jsonData,function(data){
					//检测信息
					showMsg(data);
					//检测是否在线
					try{
	...
	function systemddlRightMenuDelete(){
	    zkAjax("/systemddlAction!deleteSystemddl.action?id="+clickNode.id,function(data){
	    	//检测信息
			showMsg(data);
			//检测是否在线
			try{
	...
	function systemddlRightMenuAdd(){
		zkAjax("/systemddlAction!addSystemddl.action?id="+clickNode.id,function(data){
			//检测信息
			showMsg(data);
			//检测是否在线
			try{
