...
<script type="text/javascript">
	...
	$(function($) {
		gwlxwmglDatagrid = $('#admins_ow_gwlxwmgl_datagrid').datagrid({
			...
			url :'${pageContext.request.contextPath}'+owBackNamespace+'/owContactUsBackAction!getDatagrid.action',
			pagination:true,
			fit:true,
			idField:'id',
			sortName:'createTime',
	...
	*/
	function gwlxwmglEditStatus(index){
		...
		gwlxwmglEditStatusDialog = $('<div/>').dialog({
			...
			height:230,
			modal:true,
			href:'${pageContext.request.contextPath}/admins/officialwebsite/gwlxwmgl_editstatusdialog.jsp',
			onClose:function(){
				//删除以前的节点
				$(this).dialog('destroy');
		...
		if(checkedRows.length<1){
			...
			$.messager.confirm('提示','确认删除这'+checkedRows.length+'行？',function(r){
				if(r){
					...
					//链接服务器删除
					zkAjax(owBackNamespace+"/owContactUsBackAction!deleteContactUs.action?deleteIds="+ids.join(','),function(data){
