...
<script type="text/javascript">
	...
	$(function($) {
		gwxwglDatagrid = $('#admins_ow_gwxwgl_datagrid').datagrid({
			...
			url :'${pageContext.request.contextPath}'+owBackNamespace+'/owNewsBackAction!getDatagrid.action',
			pagination:true,
			fit:true,
			idField:'id',
			sortName:'createTime',
		...
		if(checkedRows.length<1){
			...
			$.messager.confirm('提示','确认删除这'+checkedRows.length+'行？',function(r){
				if(r){
					...
					//链接服务器删除
					zkAjax(owBackNamespace+"/owNewsBackAction!deleteNews.action?deleteIds="+ids.join(','),function(data){
	...
	*/
	function gwxwglAdd(){
		...
		gwxwglAddDialog = $('<div/>').dialog({
			...
			maximized:true,
			modal:true,
			href:'${pageContext.request.contextPath}/admins/officialwebsite/gwxwgl_adddialog.jsp',
			onClose:function(){
				//删除以前的节点
				$(this).dialog('destroy');
	...
	function gwxwglEdit(){
		...
		if(checkedRow.length==1){
			...
			gwxwglEditDialog = $('<div/>').dialog({
				...
				maximized:true,
				modal:true,
				href:'${pageContext.request.contextPath}/admins/officialwebsite/gwxwgl_editdialog.jsp',
				onClose:function(){
					//删除以前的节点
					$(this).dialog('destroy');
