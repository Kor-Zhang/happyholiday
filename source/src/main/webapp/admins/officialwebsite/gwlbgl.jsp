...
<script type="text/javascript">
	...
	$(function($) {
		gwlbglDatagrid = $('#admins_ow_gwlbgl_datagrid').datagrid({
			...
			url :'${pageContext.request.contextPath}'+owBackNamespace+'/owIndexCarouselBackAction!getDatagrid.action',
			pagination:true,
			fit:true,
			idField:'id',
			sortName:'createTime',
			...
			columns : [ [ {
				...
				formatter: function(value,row,index){
					var carImg = "<span style='width:200px;' title='点击更新头像'><img style='width:200px;' src= ${pageContext.request.contextPath}"+owBackNamespace+"/owIndexCarouselBackAction!getImgByImgName.action?imgName="+value+"&t="+new Date().getTime()+"'/></span>";
					return carImg;
				}
			},{
				field : 'introduction',
	...
	*/
	function gwlbglAdd(){
		...
		gwlbglAddDialog = $('<div/>').dialog({
			...
			maximizable:true,
			modal:true,
			href:'${pageContext.request.contextPath}/admins/officialwebsite/gwlbgl_adddialog.jsp',
			onClose:function(){
				//删除以前的节点
				$(this).dialog('destroy');
	...
	function gwlbglEdit(){
		...
		if(checkedRow.length==1){
			...
			gwlbglEditDialog = $('<div/>').dialog({
				...
				height:230,
				modal:true,
				href:'${pageContext.request.contextPath}/admins/officialwebsite/gwlbgl_editdialog.jsp',
				onClose:function(){
					//删除以前的节点
					$(this).dialog('destroy');
		...
		if(checkedRows.length<1){
			...
			$.messager.confirm('提示','确认删除这'+checkedRows.length+'行？',function(r){
				if(r){
					...
					//链接服务器删除
					zkAjax(owBackNamespace+"/owIndexCarouselBackAction!deleteIndexCarousel.action?deleteIds="+ids.join(','),function(data){
