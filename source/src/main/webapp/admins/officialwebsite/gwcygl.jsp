...
<script type="text/javascript">
	...
	$(function($) {
		gwcyglDatagrid = $('#admins_ow_gwcygl_datagrid').datagrid({
			...
			url :'${pageContext.request.contextPath}'+owBackNamespace+'/owMembersBackAction!getDatagrid.action',
			pagination:true,
			fit:true,
			idField:'id',
			sortName:'createTime',
			...
			columns : [ [ {
				...
				formatter: function(value,row,index){
					var carImg = "<span style='width:150px;height:150px;' title=''><img style='width:150px;height:150px;' src= ${pageContext.request.contextPath}"+owBackNamespace+"/owMembersBackAction!getImgByHead.action?head="+value+"&t="+new Date().getTime()+"'/></span>";
					return carImg;
				}
			},{
				field : 'name',
	...
	*/
	function gwcyglAdd(){
		...
		gwcyglAddDialog = $('<div/>').dialog({
			...
			maximizable:true,
			modal:true,
			href:'${pageContext.request.contextPath}/admins/officialwebsite/gwcygl_adddialog.jsp',
			onClose:function(){
				//删除以前的节点
				$(this).dialog('destroy');
	...
	function gwcyglEdit(){
		...
		if(checkedRow.length==1){
			...
			gwcyglEditDialog = $('<div/>').dialog({
				...
				height:300,
				modal:true,
				href:'${pageContext.request.contextPath}/admins/officialwebsite/gwcygl_editdialog.jsp',
				onClose:function(){
					//删除以前的节点
					$(this).dialog('destroy');
		...
		if(checkedRows.length<1){
			...
			$.messager.confirm('提示','确认删除这'+checkedRows.length+'行？',function(r){
				if(r){
					...
					//链接服务器删除
					zkAjax(owBackNamespace+"/owMembersBackAction!deleteMembers.action?deleteIds="+ids.join(','),function(data){
