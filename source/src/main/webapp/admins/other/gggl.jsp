...
<script type="text/javascript">
	...
	$(function($) {
		ggglDatagrid = $('#admins_gggl_datagrid').datagrid({
			...
			url :'${pageContext.request.contextPath}/noticesAction!getNoticesDatagrid.action',
			pagination:true,
			fit:true,
			idField:'id',
			sortName:'createTime',
	...
	*/
	function ggglAdd(){
		...
		ggglAddDialog = $('<div/>').dialog({
			...
			maximizable:true,
			modal:true,
			href:'${pageContext.request.contextPath}/admins/other/gggl_adddialog.jsp',
			onClose:function(){
				//删除以前的节点
				$(this).dialog('destroy');
	...
	function ggglEdit(){
		...
		if(checkedRow.length==1){
			...
			ggglEditDialog = $('<div/>').dialog({
				...
				height:400,
				modal:true,
				href:'${pageContext.request.contextPath}/admins/other/gggl_editdialog.jsp',
				onClose:function(){
					//删除以前的节点
					$(this).dialog('destroy');
		...
		if(checkedRows.length<1){
			...
			$.messager.confirm('提示','确认删除这'+checkedRows.length+'行？',function(r){
				if(r){
					...
					//链接服务器删除
					zkAjax("/noticesAction!deleteNotices.action?deleteIds="+ids.join(','),function(data){
