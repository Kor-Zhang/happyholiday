...
public class AdminsServiceImpl implements AdminsServiceI {
	...
	@Override
	public BackReturnJSON<PageAdmins> getDatagrid(Admins onlineAdmin,PageAdmins pageAdmin) throws Exception {
		/**
		 * 1.判断当前用户是否拥有查询权限
		 * 2.执行
		 * 3.返回结果
	...
	@Override
	public Admins addAdmin(Admins onlineAdmin,PageAdmins pageAdmin) throws Exception {
		//1.验证202权限（添加管理员权限）
	...
	@Override
	public void deleteAdmins(Admins onlineAdmin,String deleteIds) throws Exception{
		/**
	...
	@Override
	public PageAdmins editAdmin(Admins onlineAdmin,PageAdmins pageAdmin) throws Exception {
		/**
		 * 不更新id和password
		 * 1.判断是否改管理员存在
		 * 2.执行操作
	...
	public void updateAdminPwd(Admins onlineAdmin,PageAdmins pageAdmin) throws Exception{
		//验证204权限（跟新管理员权限）
		if(!BackTools.power(onlineAdmin.getId(),BackStatic.POWER_ID_UPDATE_ADMIN)){
			throw new AdminsException("您没有更新管理员的权限！");
		}
	...
	public BackReturnJSON<PageAdmins> getSingleAdminDatagridById(
			Admins onlineAdmin, PageAdmins pageAdmin) throws Exception {
		/**
		 * 1.判断当前用户是否拥有查询权限
		 * 2.执行
		 * 3.返回结果
